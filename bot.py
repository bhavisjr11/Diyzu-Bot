...
from telegram import (
    ...
    ReplyKeyboardMarkup, MessageEntity
)
...
#  CONFIG
# ═══════════════════════════════════════════════════════════════════════════════
BOT_TOKEN       = os.environ.get("BOT_TOKEN", "8564888521:AAGspgYdZHQa35qN06PXwmv9zCUVxuRe0rg")
...
CE_BNB_ICO    = ce("5951784276558093439", "💵")

CE_PAY_HDR    = ce("5256047523620995497", "💳")
CE_LOCAL_GW   = ce("5848290240627740402", "🏦")
CE_CRYPTO_GW  = ce("5938168246682259511", "🔐")
CE_BNB_PM     = ce("5285185333376325970", "💵")
CE_BYBIT_PM = ce("5206489137238002865", "🔶")
CE_BITGET_PM  = ce("5411167410046653046", "🔷")
CE_INTL_GW    = ce("5285452600601237916", "🌐")
CE_BGIFTCARD  = ce("6147764669361692707", "🎁")
CE_REMITLY    = ce("5044471864047436992", "💸")
CE_TAPTAP     = ce("5213403875670765022", "📲")
CE_INFO       = ce("5951675283173023271", "📋")
CE_PAY_CLOSE  = ce("5215492745900077682", "✨")
...
def kb_buy_methods(k: str, idx: int, dur_enc: str, price: str, d: dict) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton(f"₿  Binance Pay",                               callback_data=f"pay|bnb|{base}")],
    ]
    if upi_qr:
        rows.append([InlineKeyboardButton("  UPI Payment",                   callback_data=f"pay|upi|{base}")])
    rows += [
        [InlineKeyboardButton(" Local Gateways (Bangladesh)",   callback_data=f"pm|local|{base}")],
        [InlineKeyboardButton(" Crypto Exchanges & Wallets",    callback_data=f"pm|crypto|{base}")],
        [InlineKeyboardButton(" International Gateways",        callback_data=f"pm|intl|{base}")],
        [InlineKeyboardButton(" Remitly",                       callback_data=f"pm|remitly|{base}")],
        [InlineKeyboardButton(" TAPTAP [BANGLADESH TAKA]",      callback_data=f"pm|taptap|{base}")],
...
def _strip_html(html: str) -> str:
    ...
    plain = _re.sub(r'<tg-emoji[^>]*>|</tg-emoji>', '', html)
    plain = _re.sub(r'<[^>]+>', '', plain)
    return plain.replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>').replace('&quot;', '"')

async def _send_html(msg_obj, html: str, **kwargs):
    """Try sending with HTML (custom emoji). Falls back to plain text if rejected."""
    from telegram.error import BadRequest
    try:
        await msg_obj.reply_text(html, parse_mode="HTML", **kwargs)
    except BadRequest:
        await msg_obj.reply_text(_strip_html(html), **kwargs)

def payment_methods_msg() -> str:
    return (
        CE_PAY_HDR + " <b>Available Payment Methods</b>\n\n"
        "I accept payments through the following methods for your convenience 👉\n\n"
        + _SEP
        + CE_LOCAL_GW + " <b>Local Gateways (Bangladesh):</b>\n"
        + _SEP
        + CE_LOCAL_GW + " <b>BKASH</b>   :  <code>01842485666</code>\n"
        + CE_LOCAL_GW + " <b>NAGAD</b>   :  <code>01950728360</code>\n"
        + CE_LOCAL_GW + " <b>ROCKET</b>  :  <code>01950728360</code>\n\n"
        + _SEP
        + CE_CRYPTO_GW + " <b>Crypto Exchanges &amp; Wallets:</b>\n"
        + _SEP
        + CE_BNB_PM   + " <b>Binance</b>  :  <code>865770026</code>\n"
        + CE_BYBIT_PM + " <b>Bybit</b>    :  <code>376441126</code>\n"
        + CE_BITGET_PM+ " <b>Bitget</b>   :  <code>2498504246</code>\n\n"
        + _SEP
        + CE_INTL_GW + " <b>International Gateways:</b>\n"
        + _SEP
        + CE_BGIFTCARD + " <b>Binance Gift Card</b>\n"
        "   👉 <a href='https://www.g2a.com/page/cart'>g2a.com/page/cart</a>\n\n"
        + CE_REMITLY + " <b>Remitly</b>\n"
        + CE_INFO + "   NAME   : <b>DIYZU CHEAT</b>\n"
        + CE_INFO + "   NUMBER : <code>01842485666</code>\n\n"
        + CE_TAPTAP + " <b>TAPTAP [BANGLADESH TAKA]</b>\n"
        + CE_INFO + "   NAME   : <b>DIYZU CHEAT</b>\n"
        + CE_INFO + "   NUMBER : <code>01842485666</code>\n\n"
        + _SEP
        + CE_PAY_CLOSE + " Choose the one that's most comfortable for you and enjoy hassle-free transactions with me."
    )

def payment_methods_local_msg() -> str:
    return (
        CE_LOCAL_GW + " <b>Local Gateways (Bangladesh)</b>\n\n"
        + CE_LOCAL_GW + " <b>BKASH</b>   :  <code>01842485666</code>\n"
        + CE_LOCAL_GW + " <b>NAGAD</b>   :  <code>01950728360</code>\n"
        + CE_LOCAL_GW + " <b>ROCKET</b>  :  <code>01950728360</code>\n\n"
        "Send payment then click <b>I've Sent Payment</b> below."
    )

def payment_methods_crypto_msg() -> str:
    return (
        CE_CRYPTO_GW + " <b>Crypto Exchanges &amp; Wallets</b>\n\n"
        + CE_BNB_PM   + " <b>Binance</b>  :  <code>865770026</code>\n"
        + CE_BYBIT_PM + " <b>Bybit</b>    :  <code>376441126</code>\n"
        + CE_BITGET_PM + " <b>Bitget</b>   :  <code>2498504246</code>\n\n"
        "Send USDT then click <b>I've Sent Payment</b> below."
    )

def payment_methods_intl_msg() -> str:
    return (
        CE_INTL_GW + " <b>International Gateways</b>\n\n"
        + CE_BGIFTCARD + " <b>Binance Gift Card</b>\n"
        "   👉 <a href='https://www.g2a.com/page/cart'>g2a.com/page/cart</a>\n\n"
        "After payment click <b>I've Sent Payment</b> below."
    )

def payment_methods_remitly_msg() -> str:
    return (
        CE_REMITLY + " <b>Remitly</b>\n\n"
        + CE_INFO + " NAME   : <b>DIYZU CHEAT</b>\n"
        + CE_INFO + " NUMBER : <code>01842485666</code>\n\n"
...
def payment_methods_taptap_msg() -> str:
    return (
        CE_TAPTAP + " <b>TAPTAP [BANGLADESH TAKA]</b>\n\n"
        + CE_INFO + " NAME   : <b>DIYZU CHEAT</b>\n"
        + CE_INFO + " NUMBER : <code>01842485666</code>\n\n"
...
def kb_payment_methods_nav() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(" Local Gateways (Bangladesh)",   callback_data="pm|local")],
        [InlineKeyboardButton(" Crypto Exchanges & Wallets",    callback_data="pm|crypto")],
        [InlineKeyboardButton(" International Gateways",        callback_data="pm|intl")],
        [InlineKeyboardButton(" Remitly",                       callback_data="pm|remitly")],
        [InlineKeyboardButton(" TAPTAP [BANGLADESH TAKA]",      callback_data="pm|taptap")],
    ])
...
async def handle_media(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state and state.startswith("add_file_item|") and is_admin(uid, d):
        ...
        await update.message.reply_text(
            ...
            f"This file will be sent to <b>every buyer</b> of this product.",
            parse_mode="HTML"
        )
        return
    ...
    if state and state.startswith("waiting_ss|"):
        ...
        method_label = {
            ...
            "local":   "Local Gateway (Bangladesh)",
            "crypto":  "Crypto Exchange / Wallet",
            "intl":    "International Gateway",
            "remitly": "Remitly",
            "taptap":  "TAPTAP",
        }.get(wmethod, wmethod)
        user_info = (
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    menu_buttons = ["Shop", "Account", "Stock", "Reseller", "Admin Panel", "💳 Payment Methods"]
    if text in menu_buttons:
        if state:
            clear_state(d, uid)
        state = None

    # ── Payment Methods — shown first so it always works regardless of state ───
    if text == "💳 Payment Methods":
        await _send_html(update.message, payment_methods_msg(),
    ...
    if state and state.startswith("add_file_item|") and is_admin(uid, d):
        ...
        await update.message.reply_text(
            ...
            f"This link will be sent to <b>every buyer</b> of this product.",
            parse_mode="HTML"
        )
        return
...
async def _handle_cb_inner(q, uid: int, cb: str, d: dict, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Payment Methods (inline — from purchase screen OR menu nav) ───────────
    if cb.startswith("pm|"):
        parts     = cb.split("|", 2)
        action    = parts[1] if len(parts) > 1 else "all"
        back_base = parts[2] if len(parts) > 2 else ""

        # Parse purchase context if present
        k_b = i_b = dur_b = dur_decoded = price_b = ""
        has_ctx = False
        if back_base:
            bp = back_base.split("|")
            if len(bp) >= 4:
                k_b, i_b, dur_b, price_b = bp[0], bp[1], bp[2], bp[3]
                dur_decoded = dec_dur(dur_b)
                has_ctx = True

        def purchase_kb(method: str) -> InlineKeyboardMarkup | None:
            if not has_ctx:
                return None
            return InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    "✅  I've Sent Payment",
                    callback_data=f"paid|{k_b}|{i_b}|{dur_decoded}|{price_b}|{method}"
                )],
                [InlineKeyboardButton(
                    "⬅️  Back to Payment Options",
                    callback_data=f"dur|{k_b}|{i_b}|{dur_b}|{price_b}"
                )],
            ])

        # Map action → (message_fn, method_label)
        gw_map = {
            "local":   (payment_methods_local_msg,   "local"),
            "crypto":  (payment_methods_crypto_msg,  "crypto"),
            "intl":    (payment_methods_intl_msg,    "intl"),
            "remitly": (payment_methods_remitly_msg, "remitly"),
            "taptap":  (payment_methods_taptap_msg,  "taptap"),
        }

        if action in gw_map:
            msg_fn, method_key = gw_map[action]
            kb = purchase_kb(method_key)
            await _send_html(q.message, msg_fn(),
                             disable_web_page_preview=True,
                             reply_markup=kb)
            return

        # action == "all" — show full combined message (no purchase ctx)
        await _send_html(q.message, payment_methods_msg(),
                         disable_web_page_preview=True)
        return
    ...
    if cb.startswith("dur|"):
        ...
        if qty == 0:
            ...
            note = f"\n⚠️ Balance low — choose another payment method below" if bal < float(price) else ""
            txt = (
                f"{_ce} <b>{esc(pname)}</b>\n"
                f"{CE_DUR_ICO} Duration: <b>{esc(dur)}</b>  |  {CE_PRICE_ICO} Price: <b>${esc(price)}</b>\n"
    ...
    if cb.startswith("afp|"):
        ...
        existing = peek_file(cat_key, idx, d)
        status   = f"✅ <b>{esc(existing['name'])}</b>" if existing else "❌ None"
        set_state(d, uid, f"add_file_item|{cat_key}|{idx}")
        await q.edit_message_text(
            f"📁 <b>Set File — {esc(pname)}</b>\n\n"
            f"Current file: {status}\n\n"
            f"Send a <b>file</b> (APK / IPA / ZIP / any) or a <b>download link</b> as text.\n"
            f"<i>This will replace any existing file. All buyers get the same file.</i>",
            parse_mode="HTML")
        return
